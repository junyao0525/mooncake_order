...
export default function OrderForm({ agent }: { agent: Agent }) {
...
function Header() {
  return (
    <header className="relative bg-gradient-to-b from-[#f3d9a3] to-cream-soft px-5 py-8 text-center sm:px-8 sm:py-10">
      ...
      {/* The logo already carries the tagline, "Angel Bakery" and 天使牌, so
          it replaces those lines rather than sitting above a repeat of them. */}
      <h1>
        <Image
          src="/logo.png"
          alt="Angel Bakery 天使牌 — Every Sweet Come From Angel · Handmade"
          width={1072}
          height={1096}
          priority
          // Rendered at h-28 / sm:h-40, so tell Next not to ship the 1080px
          // variant for a ~160px slot.
          sizes="(min-width: 640px) 160px, 112px"
          className="mx-auto h-28 w-auto drop-shadow-sm sm:h-40"
        />
      </h1>
      <div className="mt-2 font-display text-xl font-bold tracking-wide text-brown-deep sm:text-3xl">
        2026 MOON CAKE
      </div>
...
/* Brand marks drawn inline so they scale with the text and inherit its
   colour — no icon font, no extra image request. Paths from Simple Icons. */
